# 🌟 Awesome Etherlink

> **Curated directory of the entire Etherlink ecosystem**

![GitHub stars](https://img.shields.io/github/stars/etherlink-intern/awesome-etherlink?style=social)
![GitHub forks](https://img.shields.io/github/forks/etherlink-intern/awesome-etherlink?style=social)
![GitHub issues](https://img.shields.io/github/issues/etherlink-intern/awesome-etherlink)
![GitHub license](https://img.shields.io/github/license/etherlink-intern/awesome-etherlink)

---

*Start exploring the comprehensive list below... This directory contains **100+ projects** across DePIN, DeFi, NFTs, Gaming, and Infrastructure.*

<div align="center">
  <img src="https://avatars.githubusercontent.com/u/169287244?s=200&v=4" alt="Etherlink Logo" width="120">
</div>

> A curated list of awesome projects, tools, and resources in the Etherlink ecosystem

**Etherlink** is an EVM-compatible, non-custodial Layer 2 blockchain powered by Tezos Smart Rollup technology, offering high-performance scaling, low transaction costs, and seamless interoperability with Ethereum. This comprehensive list showcases the rapidly growing ecosystem of projects, developer tools, DeFi protocols, gaming platforms, and infrastructure that make Etherlink a leading destination for blockchain innovation.

## Contents

- [Official Resources](#official-resources)
- [Core Infrastructure](#core-infrastructure)
- [Developer Tools](#developer-tools)
- [DeFi Protocols](#defi-protocols)
    - [Centralized Exchanges](#centralized-exchanges)
    - [Decentralized Exchanges](#decentralized-exchanges)
    - [Lending & Borrowing](#lending--borrowing)
    - [Liquidity & Yield](#liquidity--yield)
    - [Asset Management](#asset-management)
    - [Real World Assets (RWA)](#real-world-assets-rwa)
    - [Prediction & Gaming DeFi](#prediction--gaming-defi)
    - [Cross-chain DeFi](#cross-chain-defi)
- [Gaming & Metaverse](#gaming--metaverse)
- [NFT & Collectibles](#nft--collectibles)
- [Wallets](#wallets)
- [Identity & Social](#identity--social)
- [Communication Tools](#communication-tools)
- [Cross-chain & Bridges](#cross-chain--bridges)
- [Analytics & Monitoring](#analytics--monitoring)
    - [Blockchain Analytics](#blockchain-analytics)
    - [Block Explorers](#block-explorers)
- [AI & DePIN](#ai--depin)
- [Official GitHub Projects](#official-github-projects)
- [Resources](#resources)

## Official Resources

### Core Platforms
- **[Etherlink Official](https://etherlink.com)** - Official website and main portal
- **[📖 Documentation](https://docs.etherlink.com/)** - Comprehensive developer documentation
- **[Explorer](https://explorer.etherlink.com/)** - Official block explorer
- **[Testnet Explorer](https://testnet-explorer.etherlink.com/)** - Testnet block explorer

### Community
- **[Discord](https://discord.gg/etherlink)** - Official Discord community
- **[X/Twitter](https://x.com/etherlink)** - Official Twitter account
- **[Blog](https://blog.etherlinkinsights.com/)** - Official blog with updates

## Core Infrastructure

### Oracles
- **[Pyth Network](https://pyth.network)** - Real-time market data feeds across multiple asset classes
- **[Redstone](https://redstone.finance)** - Decentralized oracle network for custom data feeds
- **[Acelon](https://acelon.io)** - Price feed provider for stXTZ tokens and staking pool data

### Indexing & Querying
- **[The Graph](https://thegraph.com)** - Decentralized indexing and querying protocol
- **[SQD](https://subsquid.io)** - Subsquid decentralized data indexing solution
- **[Subquery](https://www.subquery.network/indexer)** - Open-source indexer supporting Etherlink and multiple blockchains
- **[DipDup](https://dipdup.io)** - Indexing framework for blockchain data

### Messaging & Interoperability
- **[LayerZero](https://layerzero.network)** - Omnichain protocol for seamless cross-chain communication
- **[Stargate](https://stargate.finance)** - Unified liquidity and asset transfer protocol

### Security & Infrastructure
- **[Fireblocks](https://www.fireblocks.com)** - Enterprise-grade platform for digital asset operations
- **[Pimlico](https://www.pimlico.io)** - Account abstraction infrastructure
- **[Gelato](https://www.gelato.network)** - Automated smart contract execution network
- **[Li.Fi](https://li.fi)** - Unified cross-chain swapping and bridging

### RPC & API Providers
- **[Ankr](https://www.ankr.com)** - Distributed node infrastructure and APIs
- **[BlockPI](https://blockpi.io)** - High-performance blockchain RPC service
- **[Chainstack](https://chainstack.com)** - Enterprise-grade blockchain infrastructure platform
- **[GetBlock](https://getblock.io)** - RPC node provider with WebSocket support
- **[NOWNodes](https://nownodes.io)** - Scalable blockchain infrastructure with high uptime
- **[Zeeve](https://www.zeeve.io)** - Blockchain infrastructure as a service

## Developer Tools

### Development Frameworks
- **[Thirdweb](https://thirdweb.com)** - Complete framework for building web3 applications
- **[Tezos Unity SDK](https://docs.tezos.com/unity)** - Unity SDK for blockchain integration with Etherlink support
- **[Sequence](https://sequence.xyz)** - Gaming-focused wallet and infrastructure platform
- **[Google Cloud](https://cloud.google.com)** - Cloud infrastructure for blockchain development
- **[GroupFi](https://groupfi.ai)** - Privacy-focused blockchain infrastructure

### Smart Contract Development
- **[Hardhat](https://hardhat.org)** - Ethereum development environment (supporting Etherlink)
- **[Foundry](https://foundry.paradigm.xyz)** - Solidity testing framework (with Etherlink support)
- **[Viem](https://viem.sh)** - TypeScript interface for Ethereum (with Etherlink support)

### Wallet Infrastructure
- **[Magic.link](https://magic.link)** - Passwordless wallet infrastructure
- **[Web3Auth](https://web3auth.io)** - Wallet authentication for mainstream users

### Analytics & Data
- **[DappRadar](https://dappradar.com)** - DApp analytics and discovery platform
- **[Octav](https://octav.fi)** - Portfolio analytics and tracking
- **[Goldsky](https://goldsky.com)** - Real-time data streaming infrastructure

### Onramp & Payment
- **[Transak](https://transak.com)** - Seamless fiat-to-crypto onboarding solution
- **[Kredete](https://kredete.com)** - Cross-border payments and credit building
- **[Secret Network](https://scrt.network)** - Privacy-preserving confidential computing

## DeFi Protocols

### Centralized Exchanges
- **[AscendEx](https://ascendex.com)** - Centralized cryptocurrency exchange supporting XTZ trading

### Decentralized Exchanges
- **[Iguanadex](https://iguanadex.com/)** - Leading decentralized exchange on Etherlink
- **[Hanji](https://hanji.io)** - On-chain CLOB DEX with zero-fee limit orders
- **[TachySwap](https://tachyswap.org)** - AMM-based DEX for seamless token swaps
- **[Oku Trade](https://oku.trade)** - DeFi trading interface with Uniswap v3 & Morpho
- **[Curve Finance](https://curve.fi)** - Optimized stablecoin AMM
- **[KyberSwap](https://kyberswap.com)** - DEX aggregator and liquidity protocol
- **[Jumper.Exchange](https://jumper.exchange)** - Cross-chain liquidity aggregator

### Lending & Borrowing
- **[Superlend](https://superlend.io)** - Lending aggregator for Tezos and Etherlink
- **[Morpho Protocol](https://morpho.io)** - High-efficiency lending primitive
- **[Gearbox Protocol](https://gearbox.fi)** - Generalized leverage protocol

### Liquidity & Yield
- **[Organic Growth](https://www.organicgrowth.wtf/)** - DeFi yield optimization protocol
- **[Apple Farm](https://applefarm.xyz)** - Yield farming with staking and LP rewards

### Asset Management & Yield
- **[Midas](https://midas.app)** - Tokenized assets and delta-neutral yield
- **[TrustSwap](https://trustswap.org)** - Token locks, escrow, and DeFi services
- **[Team Finance](https://team.finance)** - Token management and liquidity locking
- **[IDRX Stablecoin](https://idrx.io)** - Rupiah-collateralized stablecoin

### Real World Assets (RWA)
- **[Uranium](https://uranium.io)** - Tokenized fractional ownership of physical uranium
- **[Spiko](https://spiko.io)** - Tokenized real-world assets

### Prediction & Gaming DeFi
- **[5050](https://5050.fi/)** - Decentralized prediction market platform

### Cross-chain DeFi
- **[Lombard Finance](https://lombard.finance)** - Liquid Bitcoin (LBTC) with native BTC yield

## Gaming & Metaverse

### AAA Gaming Titles
- **[Reaper Actual](https://reaperactual.com)** - AAA MMOFPS tactical shooter ($30.5M raised)
- **[Pikes Arena](https://pikesarena.com)** - Mobile PvP combat arena with on-chain assets
- **[Sugarverse](https://sugarverse.io)** - Cross-platform gaming universe with community rewards

### Casual & Social Gaming
- **[Appleville](https://appleville.xyz)** - Idle farming game with real rewards
- **[Uranium Miner](https://geturanium.io)** - Competitive mining game for crypto rewards
- **[Bit Hotel](https://bithotel.io)** - Social RPG metaverse with mini-games
- **[Sugar Match](https://sugarmatch.io)** - Match-3 puzzle with seasonal rewards
- **[UHT](https://uht.gamify.io)** - Gamified health and fitness platform
- **[Battlerise](https://battlerise.com)** - Tactical strategy with hero NFTs
- **[BattleTabs](https://battletabs.com)** - Browser-based tactical combat
- **[Miomi Game](https://miomi.game)** - Web3 eSports tournaments

### Gaming Infrastructure
- **[Ladder Protocol](https://ladder.top)** - AMM for instant gaming NFT liquidity

## NFT & Collectibles

### Marketplaces
- **[Rarible](https://rarible.com)** - Aggregated NFT marketplace with Etherlink support
- **[akaSwap](https://akaswap.com)** - NFT marketplace for the Tezos ecosystem

## Wallets

### Browser & Desktop Wallets
- **[MetaMask](https://metamask.io)** - Browser extension wallet
- **[Coinbase Wallet](https://wallet.coinbase.com)** - Self-custody cross-chain wallet
- **[Safe](https://safe.global)** - Multi-signature smart wallet
- **[Sequence](https://sequence.xyz)** - Gaming-focused smart wallet

### Mobile Wallets
- **[Temple Wallet](https://templewallet.com)** - Tezos wallet with Etherlink support

## Identity & Social

### Digital Identity
- **[idOS](https://idos.network/)** - Decentralized identity and reputation system

### Social & Community
- **[Layer3](https://layer3.xyz)** - Interactive quests for the Etherlink community
- **[TaskOn](https://taskon.xyz)** - Web3 task platform and bounty system
- **[Homebase](https://homebase.xyz)** - DAO management and governance
- **[GroupFi](https://groupfi.ai)** - Feeless on-chain messaging protocol
- **[Questflow](https://questflow.ai)** - Multi-agent AI orchestration layer
- **[Zealy](https://zealy.io)** - Quest-based community engagement

## Communication Tools

### Messaging & Social Integration
- **[t:connect](https://t-connect.gitbook.io)** - Telegram Mini Apps SDK for blockchain interaction


## AI & DePIN

### AI Projects
- **[Sogni AI](https://sogni.ai)** - DePIN for creative AI with decentralized GPU network

## Cross-chain & Bridges

### Native Bridges
- **[Tezos-EVM Bridge](https://bridge.tezos.com)** - Official Tezos ↔ Etherlink bridge
- **[Etherlink EVM Bridge](https://bridge.etherlink.com)** - Native Etherlink bridge


### Third-party Bridges
- **[OPEN Ticketing](https://onopen.xyz)** - NFT ticketing infrastructure

## Analytics & Monitoring

### Blockchain Analytics
- **[Dune Analytics](https://dune.com)** - Blockchain data queries and dashboards
- **[Chainspect](https://chainspect.app)** - Multi-chain analytics and metrics

### Block Explorers
- **[Blockscout](https://blockscout.com)** - Open-source blockchain explorer

## Official GitHub Projects

- **[Etherlink Bridge](https://github.com/baking-bad/etherlink-bridge)** - Bridge transport protocol by Baking Bad
- **[Rollup Bridge Indexer](https://github.com/baking-bad/rollup-bridge-indexer)** - FA bridge backend indexer
- **[CreateX](https://github.com/pcaversaccio/createx)** - Factory contract for EVM chains

## Resources

- **[Testnet Faucet](https://faucet.etherlink.com)** - Get testnet tokens for development
- **[Etherlink Docs](https://docs.etherlink.com)** - Official documentation

## Contributing

Contributions are welcome! Please read our [contributing guidelines](CONTRIBUTING.md) before submitting pull requests.

### How to Contribute

1. Fork the repository
2. Create a feature branch (`git checkout -b feature/amazing-project`)
3. Add your project with proper formatting
4. Ensure all links work and descriptions are accurate
5. Submit a pull request

### Project Requirements

- Projects must be actively maintained and operational on Etherlink
- Must have a working website or documentation
- Must provide clear value to the Etherlink ecosystem
- Should be open source where possible
- Must be genuinely integrated with Etherlink (not just planned)

### Submission Format

```markdown
- **[Project Name](https://example.com)** - Brief, clear description of the project
```

## License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

## Acknowledgements

- Etherlink development team and contributors
- Tezos Foundation for Smart Rollup technology
- All the amazing projects building on Etherlink
- The broader Ethereum and Layer 2 communities
- [Awesome Etherlink by @skullzarmy](https://github.com/skullzarmy/awesome-etherlink) - Another curated list of Etherlink ecosystem resources

---

⭐ If this list helped you discover projects on Etherlink, please give it a star and share it with the community!

🌐 Built with ❤️ for the Etherlink ecosystem
